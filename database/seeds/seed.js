...
const testPamphlets = [
    {
        ...
        description: 'Explore the hottest tech trends dominating the market this year. From AI to blockchain, we cover it all.',
        content: '<h1>Technology Trends 2024</h1><p>This comprehensive guide covers the latest innovations...</p><h2>AI Revolution</h2><p>Artificial Intelligence continues to transform industries...</p>',
    ...
    },
    {
        ...
        description: 'Proven strategies to scale your business from startup to enterprise.',
        content: '<h1>Business Scaling</h1><p>Learn the fundamentals of sustainable business growth...</p>',
    ...
    },
    {
        ...
        description: 'Complete guide to leveraging online education for professional development.',
        content: '<h1>E-Learning Guide</h1><p>Online education has revolutionized learning...</p>',
    ...
    },
    {
        ...
        description: 'Discover breakthrough innovations in modern healthcare and medical technology.',
        content: '<h1>Healthcare Tech</h1><p>Medical innovation is accelerating at unprecedented rates...</p>',
    ...
    },
    {
        ...
        description: 'Latest trends in entertainment, streaming, and digital content creation.',
        content: '<h1>Entertainment 2024</h1><p>The entertainment landscape continues to evolve...</p>',
    ...
    },
    {
        ...
        description: 'How technology is revolutionizing sports performance and fan experience.',
        content: '<h1>Sports Tech</h1><p>From wearables to AI coaching, sports technology is advancing...</p>',
    ...
    },
    {
        ...
        content: '<h1>Digital Marketing</h1><p>Effective digital marketing requires knowledge of multiple channels...</p>',
    ...
    },
    {
        ...
        content: '<h1>Cloud Computing</h1><p>Cloud infrastructure has become essential for modern businesses...</p>',
...
function seed() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            connection = yield db_js_1.default.getConnection();
            // Disable foreign key checks temporarily
            yield connection.query('SET FOREIGN_KEY_CHECKS = 0');
            // Clear existing data
            console.log('🧹 Clearing existing data...');
            yield connection.query('TRUNCATE TABLE feed');
            yield connection.query('TRUNCATE TABLE pamphlet_locations');
            yield connection.query('TRUNCATE TABLE pamphlet_contacts');
            yield connection.query('TRUNCATE TABLE pamphlet_images');
            yield connection.query('TRUNCATE TABLE pamphlets');
            ...
            for (const user of testUsers) {
                yield connection.query('INSERT INTO users (id, name, email, password) VALUES (?, ?, ?, ?)', [user.id, user.name, user.email, user.password]);
            }
            console.log(`✅ Inserted ${testUsers.length} users\n`);
            // Insert pamphlets
            console.log('📄 Inserting test pamphlets...');
            for (const pamphlet of testPamphlets) {
                yield connection.query('INSERT INTO pamphlets (id, title, description, content, image_url, category, location, user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [
                    pamphlet.id,
                    pamphlet.title,
            ...
            for (const image of testImages) {
                yield connection.query('INSERT INTO pamphlet_images (pamphlet_id, image_url) VALUES (?, ?)', [image.pamphlet_id, image.image_url]);
            }
            console.log(`✅ Inserted ${testImages.length} images\n`);
            // Insert contacts
            console.log('📞 Inserting pamphlet contacts...');
            for (const contact of testContacts) {
                yield connection.query('INSERT INTO pamphlet_contacts (pamphlet_id, phone, whatsapp, email) VALUES (?, ?, ?, ?)', [contact.pamphlet_id, contact.phone, contact.whatsapp, contact.email]);
            }
            console.log(`✅ Inserted ${testContacts.length} contacts\n`);
            // Insert locations
            console.log('📍 Inserting pamphlet locations...');
            for (const location of testLocations) {
                yield connection.query('INSERT INTO pamphlet_locations (pamphlet_id, address, latitude, longitude) VALUES (?, ?, ?, ?)', [
            ...
            const [userCount] = yield connection.query('SELECT COUNT(*) as total FROM users');
            const [pamphletCount] = yield connection.query('SELECT COUNT(*) as total FROM pamphlets');
            const [imageCount] = yield connection.query('SELECT COUNT(*) as total FROM pamphlet_images');
            const [contactCount] = yield connection.query('SELECT COUNT(*) as total FROM pamphlet_contacts');
            const [locationCount] = yield connection.query('SELECT COUNT(*) as total FROM pamphlet_locations');
            console.log(`\n📈 Seed Statistics:`);
            console.log(`   Users: ${userCount[0].total}`);
            console.log(`   Pamphlets: ${pamphletCount[0].total}`);
            console.log(`   Images: ${imageCount[0].total}`);
            console.log(`   Contacts: ${contactCount[0].total}`);
            console.log(`   Locations: ${locationCount[0].total}`);
